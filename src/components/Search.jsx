...
function Search() {
  const { setSearchValue } = React.useContext(SearchContetxt);
  ...
  return (
    <div className="w-full h-12 my-4 bg-white border-2 border-border rounded-full">
      ...
      >
        <input
          type="text"
          className="w-4/5 h-full px-4 rounded-l-full text-lg   focus:outline-none"
          ref={search}
        />
        <button className="w-1/5 h-full rounded-r-full border-l-border border-l-2 flex justify-center items-center">
